...
const char argp_program_doc[] =
	"Monitor latency to various points in the ingress network stack";
...
static const struct argp_option opts[] = {
	...
	{ "enable-probes", 'e', "PROBES", 0,
	  "comma-seperated list of probe points to include", 1 },
	{ "disable-probes", 'd', "PROBES", 0,
	  "comma-separated list of probe points to exclude", 1 },
	...
	{ "netns", 'n', "NETNS-ID", 0,
	  "network namespace identifer to monitor (-1 for all, 0 for current (default))",
	  2 },
...
err_enable_and_disable_probes:
	argp_error(state, "%s and %s are mutually exlusive, only use one\n",
			  ...
			  bool *has_rescanned)
{
	...
	if (key->ifindex) {
		format_ifindex(key->ifindex, env->translate_ifindex, buf,
			       sizeof(buf));
